...
ggplot(dist, aes(x = value)) + geom_histogram(bins = 1000) + 
  ...
  geom_vline(xintercept = -1.96, colour="red") +
  ylab("Density") + xlab("Z-Score") + scale_y_discrete(labels = NULL)
...
t.test(filter(data, condition == "fast")$dv, filter(data, condition == "slow")$dv, paired = FALSE)
...
result <- tidy(t.test(filter(data, condition == "fast")$dv, 
                      filter(data, condition == "slow")$dv, paired = FALSE))
...
# we worked out.
set.seed(1234)
data <- data.frame(mvrnorm(n, mu, mysigma, empirical = TRUE))
colnames(data) <- c("Var_1", "Var_2")

ggplot(data, aes(x = Var_1, y = Var_2)) + geom_point() + geom_smooth(method = "lm")

rcorr(data$Var_1, data$Var_2)
...
data %>% group_by(Sample) %>% summarise(Correlation = mean(Correlation))

ggplot(data, aes(x = Correlation)) + geom_histogram(bins = 10)
...
# Create the data
data <- as.data.frame(matrix(c(34,7,13,1,4,13,7,84,24,7,2,27,51,3,23), nrow = 3))
...
sent <- get_sentiments("nrc")   #nrc for details - bing for positive vs negatives sentiment
